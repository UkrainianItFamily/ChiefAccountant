...
"use strict";
/*!*********************************************!*\
  !*** ./resources/js/report/createReport.js ***!
  \*********************************************/
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var axios__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! axios */ "./node_modules/axios/index.js");
/* harmony import */ var axios__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(axios__WEBPACK_IMPORTED_MODULE_0__);

var appsWindow = document.querySelector('#appsWindow');
var showAppsWindowBtn = document.querySelector('#openAppsWindow');

showAppsWindowBtn.onclick = function () {
  appsWindow.style.display = 'block';
  fetchDocuments();
  appsWindow.querySelectorAll('#closeModal').forEach(function (btn) {
...
function fetchDocuments() {
  ...
  axios__WEBPACK_IMPORTED_MODULE_0___default().post("/api/apps?page=".concat(page)).then(function (res) {
    formatList(res.data);
  });
}

function formatList(data) {
  var list = appsWindow.querySelector('#list');
  var paginateListBlock = appsWindow.querySelector('#paginateList');
  ...
  data.data.forEach(function (post) {
    list.innerHTML += "\n                <li style=\"cursor: pointer\" class=\"list-item my-2 p-2 border-botom\" data-id=\"".concat(post.id, "\" data-type=\"").concat(post.type, "\" data-title=\"").concat(post.title, "\">\n                    <span>").concat(post.title, "</span>\n                </li>\n            ");
  });

  for (var i = 1; i < data.links.length - 1; i++) {
    if (data.links[i].label == data.current_page) {
      paginateListBlock.innerHTML += "\n                <li style=\"cursor: pointer; color: #a52834\" class=\"mx-2\" class='paginateLink' data-page='".concat(data.links[i].label, "'>\n                    <span>").concat(data.links[i].label, "</span>\n                </li>\n            ");
    } else {
      paginateListBlock.innerHTML += "\n                <li style=\"cursor: pointer\" class=\"mx-2\" class='paginateLink' data-page='".concat(data.links[i].label, "'>\n                    <span>").concat(data.links[i].label, "</span>\n                </li>\n            ");
    }
  }

  appsWindow.querySelector('#paginateList').onclick = function (event) {
    var paginateListItem = event.target.closest('li');
    var pageNumber = paginateListItem.dataset['page'];
    fetchDocuments(pageNumber);
  };

  list.onclick = function (event) {
    var appsListItem = event.target.closest('li');

    if (appsWindow.querySelector('#msg').firstChild) {
      hideMsg();
    }

    document.querySelector('#apps').querySelectorAll('li').forEach(function (app) {
      if (app.id == appsListItem.dataset['id']) {
        generateError('Уже есть в списке прикреплённых');
        app.remove();
      }
    });
    document.querySelector('#apps').innerHTML += "\n            <li class=\"my-2 p-2 appListItem\" id=\"".concat(appsListItem.dataset['id'], "\">\n                <input type=\"checkbox\"\n                    name=\"appsIdArray[]\"\n                    value=\"").concat(appsListItem.dataset['id'], "\"\n                    hidden\n                    checked\n                >\n                    <span>").concat(appsListItem.dataset['title'], "</span>\n                    <span class=\"mx-4\" id=\"deleteApp\" style=\"cursor: pointer;\">X</span>\n            </li>\n        ");
    generateSuccessMsg('Добавлено');
  };
}

document.querySelector('#apps').onclick = function (event) {
  var deleteBtn = event.target.closest('#deleteApp');
  var li = deleteBtn.closest('.appListItem');
  li.remove();
};
