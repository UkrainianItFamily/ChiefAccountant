...
function tagin(el, option = {}) {
    ...
    ; (function () {
        ...
        const template = `<div class="">${tags}<input type="text" class="bg-light w-100 ${classInput}" placeholder="${placeholder}"></div>`
    ...
    document.addEventListener('click', e => {
        if (e.target.closest('.' + classRemove)) {
            ...
            updateValue()
            tags = getValues().map(templateTag).join('')
            let tegsVal = document.querySelectorAll('#tags');
        }
    })
